...
//   const [isModelYPage, setIsModelYPage] = useState(false);
...
//     setIsModelYPage(path.includes("modely/learn-more"));
//   }, []);
...
//   if (isModelYPage && isScrolled) {
//     return (
//       <nav className="fixed top-0 left-0 w-full z-50 bg-black/30 backdrop-blur-md">
//         <div className="max-w-[1440px] mx-auto px-4 py-3 flex justify-between items-center">
//           <Link to="/modely" className="text-white text-sm font-semibold">
//             Model Y
//           </Link>

//           <div className="hidden md:flex items-center space-x-6">
//             <Link to="/modely/demo-drive" className="text-sm font-medium text-white hover:text-gray-300">
//               Demo Drive
//             </Link>
...
import { HelpCircle, Globe, User, Menu, X } from "lucide-react";
import NavMenuTrigger from "../custom/Navbar/NavMenuTrigger";
import ModelSpecificNav from "../custom/Navbar/ModelSpecificNav";
import { navMenus } from "../../data/navLinks";

const TeslaLogo: React.FC = () => (
  <svg className="h-3 w-28" viewBox="0 0 342 35" xmlns="http://www.w3.org/2000/svg">
    <path 
...
const Navbar: React.FC = () => {
  ...
  // Check specific pages for conditional navigation rendering
  const isModel3LearnMorePage = location.pathname.includes("model3/learn-more");
  const isModelYPage = location.pathname.includes("modely/learn-more");
  ...
  // Model specific pages navigation rendering
  if (isModel3LearnMorePage && isScrolled) {
    const model3Links = [
      { title: "Experience Model 3", path: "/model3/learn-more" },
      { title: "Compare", path: "/model3/compare" },
      { title: "Trade In", path: "/trade-in" },
      { title: "Order Now", path: "/model3" }
    ];
  ...
  if (isModelYPage && isScrolled) {
    const modelYLinks = [
      { title: "Demo Drive", path: "/modely/demo-drive" },
      { title: "Order Now", path: "/modely/order-now", isButton: true }
    ];
    ...
    <div className="relative">
        ...
        <div className="hidden md:flex gap-4 lg:gap-6 text-sm font-medium">
          {Object.entries(navMenus).map(([key, menu]) => (
            <NavMenuTrigger
              ...
              links={menu.links}
              resourceLinks={menu.resourceLinks}
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 top-14 z-40 bg-white p-4 overflow-y-auto md:hidden">
          <div className="flex flex-col space-y-4">
            {Object.entries(navMenus).map(([key, menu]) => (
              <Link key={key} to={`/${key.toLowerCase()}`} className="py-2 px-4 text-lg border-b">
                {menu.title}
              </Link>
            ))}
            ...
            <div className="py-4">
              <h3 className="text-lg font-medium mb-2">Vehicles</h3>
              <div className="grid grid-cols-2 gap-4">
                {navMenus.vehicles.links.map((link, index) => (
                  <Link 
                    key={index} 
                    to={link.path} 
                    className="flex flex-col items-center"

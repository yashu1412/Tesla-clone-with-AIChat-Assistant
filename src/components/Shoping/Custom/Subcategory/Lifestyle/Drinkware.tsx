...
const Drinkware: React.FC = () => {
    ...
    const fetchDrinkware = async () => {
      try {
        ...
        const response = await axios.get(
          `${API_BASE_URL}/product/subcategory/Drinkware`
        );

        if (response.data?.success && Array.isArray(response.data.products)) {
          setProducts(response.data.products);
        } else {
          setError('Failed to load drinkware');
        }
      } catch (err) {
        console.error('Error fetching drinkware:', err);
        setError('An error occurred while fetching drinkware');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleAddToCart = (product: Product) => {
    addToCart({
      id: product.id,
      name: product.name,
      price: product.price, // keep as string to match CartItem type
      image: product.image,
      quantity: 1
    });
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <p className="text-gray-600 max-w-3xl mx-auto">
              Premium bottles, mugs, and drinkware designed for Tesla fans.
            </p>
          </motion.div>
          ...
          {!loading && !error && products.length === 0 && (
            <div className="text-center py-12 bg-gray-50 rounded-lg">
              <h3 className="text-xl text-gray-600">
                No drinkware available at the moment
              </h3>
              <button
                onClick={() => navigate('/Tesla-shop')}
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="text-center p-4">
                  ...
                  <h3 className="font-medium mb-2">Premium Build</h3>
                ...
                <div className="text-center p-4">
                  ...
                  <h3 className="font-medium mb-2">Tesla Style</h3>
                  <p className="text-gray-600 text-sm">

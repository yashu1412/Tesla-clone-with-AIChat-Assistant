...
const OutdoorTech: React.FC = () => {
  ...
  useEffect(() => {
    const fetchOutdoorTech = async () => {
      try {
        setLoading(true);
        setError(null);

        const response = await axios.get(
          `${API_BASE_URL}/product/subcategory/OutdoorTech`
        );

        if (response.data?.success && Array.isArray(response.data.products)) {
          setProducts(response.data.products);
        } else {
          setError('Failed to load Outdoor Tech products');
        }
      } catch (err) {
        console.error('Error fetching Outdoor Tech products:', err);
        setError('An error occurred while fetching Outdoor Tech products');
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <p className="text-gray-600 max-w-3xl mx-auto">
              Discover innovative outdoor technology gadgets to enhance your
          ...
          {!loading && !error && products.length === 0 && (
            <div className="text-center py-12 bg-gray-50 rounded-lg">
              <h3 className="text-xl text-gray-600">
                No outdoor tech products available at the moment
              </h3>
              <button
                onClick={() => navigate('/Tesla-shop')}
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="text-center p-4">
                  ...
                  <h3 className="font-medium mb-2">Adventure Ready</h3>
                  <p className="text-gray-600 text-sm">
                    Tools and gadgets designed for outdoor enthusiasts
                  </p>
                </div>
                <div className="text-center p-4">
                  <div className="w-16 h-16 bg-black rounded-full flex items-center justify-center mx-auto mb-4">
                  ...
                  <h3 className="font-medium mb-2">Innovative</h3>
                  <p className="text-gray-600 text-sm">
                    Cutting-edge technology for convenience and efficiency
                  </p>
                </div>
                <div className="text-center p-4">
                  <div className="w-16 h-16 bg-black rounded-full flex items-center justify-center mx-auto mb-4">
                  ...
                  <p className="text-gray-600 text-sm">
                    Built to withstand outdoor conditions and adventures
                  </p>
                </div>
              </div>
            </motion.div>

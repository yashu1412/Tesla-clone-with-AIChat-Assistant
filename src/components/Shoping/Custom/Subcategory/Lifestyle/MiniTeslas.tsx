...
const MiniTeslas: React.FC = () => {
  ...
  useEffect(() => {
    const fetchMiniTeslas = async () => {
      try {
        setLoading(true);
        setError(null);

        const response = await axios.get(
          `${API_BASE_URL}/product/subcategory/MiniTeslas`
        );

        if (response.data?.success && Array.isArray(response.data.products)) {
          setProducts(response.data.products);
        } else {
          setError('Failed to load Mini Teslas');
        }
      } catch (err) {
        console.error('Error fetching Mini Teslas:', err);
        setError('An error occurred while fetching Mini Teslas');
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <p className="text-gray-600 max-w-3xl mx-auto">
              Explore our collectible range of Mini Tesla cars for enthusiasts of all ages.
            </p>
          </motion.div>
          ...
          {!loading && !error && products.length === 0 && (
            <div className="text-center py-12 bg-gray-50 rounded-lg">
              <h3 className="text-xl text-gray-600">
                No Mini Tesla models available at the moment
              </h3>
              <button
                onClick={() => navigate('/Tesla-shop')}
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="text-center p-4">
                  ...
                  <h3 className="font-medium mb-2">Collectible</h3>
                  <p className="text-gray-600 text-sm">
                    Perfect for collectors and Tesla fans alike
                  </p>
                </div>
                ...
                <div className="text-center p-4">
                  ...
                  <p className="text-gray-600 text-sm">
                    Detailed and realistic Mini Tesla models
                  </p>
                </div>
                ...
                <div className="text-center p-4">
                  ...
                  <h3 className="font-medium mb-2">Fun & Educational</h3>
                  <p className="text-gray-600 text-sm">
                    Great for kids and enthusiasts to learn about Tesla cars
                  </p>
                </div>
              </div>
            </motion.div>

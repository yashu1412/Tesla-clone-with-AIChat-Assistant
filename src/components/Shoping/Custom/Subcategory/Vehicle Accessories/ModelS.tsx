    ...
    const fetchModelS = async () => {
      try {
        ...
        const response = await axios.get(
          `${API_BASE_URL}/product/subcategory/ModelS`
        );

        if (response?.data?.success) {
          setProducts(response.data.products || []);
        } else {
          setError('Failed to load Model S products');
        }
      } catch (err) {
        console.error('Error fetching Model S products:', err);
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <p className="text-gray-600 max-w-3xl mx-auto">
              Premium accessories engineered exclusively for Tesla Model S.
            </p>
          </motion.div>
          ...
          {!loading && !error && products.length > 0 && (
            ...
            >
              <h2 className="text-2xl font-semibold mb-6 text-center">
                Why Choose Tesla Model S Accessories?
              </h2>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
                <div>
                  <h3 className="font-medium mb-2">Perfect Fit</h3>
                  <p className="text-gray-600 text-sm">
                    Designed specifically for Model S precision.
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-gray-600 text-sm">
                    Guaranteed compatibility & safety.
                  </p>
                </div>
              </div>
            </motion.div>

    ...
    const fetchModelX = async () => {
      try {
        ...
        const response = await axios.get(
          `${API_BASE_URL}/product/subcategory/ModelX`
        );

        if (response?.data?.success) {
          setProducts(response.data.products || []);
        } else {
          setError('Failed to load Model X products');
        }
      } catch (err) {
        console.error('Error fetching Model X products:', err);
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <p className="text-gray-600 max-w-3xl mx-auto">
              Premium accessories engineered exclusively for Tesla Model X.
            </p>
          </motion.div>
          ...
          {!loading && !error && products.length > 0 && (
            ...
            >
              <h2 className="text-2xl font-semibold mb-6 text-center">
                Why Choose Tesla Model X Accessories?
              </h2>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
                <div>
                  <h3 className="font-medium mb-2">Luxury Comfort</h3>
                  <p className="text-gray-600 text-sm">
                    Designed to match the premium feel of Model X.
                  </p>
                </div>

                <div>
                  <h3 className="font-medium mb-2">Heavy-Duty Build</h3>
                  <p className="text-gray-600 text-sm">
                ...
                <div>
                  <h3 className="font-medium mb-2">Tesla Approved</h3>
                  <p className="text-gray-600 text-sm">

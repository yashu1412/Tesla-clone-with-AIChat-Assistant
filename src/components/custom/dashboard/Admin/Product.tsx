...
const ProductPage: React.FC = () => {
    ...
    try {
      ...
      if (editMode && currentProduct) {
        const response = await axios.put(
          `http://localhost:4001/api/v1/product/${currentProduct.id}`,
          productData,
          config
        );
        ...
        const response = await axios.post(
          'http://localhost:4001/api/v1/product/CreateProduct',
          productData,
          config
        );
  ...
  const handleDelete = async (id: string) => {
    ...
    if (window.confirm('Are you sure you want to delete this product?')) {
      try {
        const response = await axios.delete(`http://localhost:4001/api/v1/product/${id}`, {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
    ...
    <>
      ...
      <Box 
        sx={{ 
          ...
          bgcolor: '#FFFFFF', 
          minHeight: 'calc(100vh - 64px)', // Adjust minHeight calculation
          color: '#000000',
          position: 'relative', // Ensure proper stacking context
          zIndex: 0 // Keep content below navbar
        }}
      >
        ...
        <TableContainer component={Paper} sx={{ bgcolor: '#FAFAFA', color: '#000000', boxShadow: '0px 4px 12px rgba(0, 0, 0, 0.1)' }}>
          <Table>
